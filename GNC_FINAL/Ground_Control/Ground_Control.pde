...
String serialPortName = "COM3"; // Make sure this matches your Ground COM port
int baudRate = 115200;
float altMin = 0;
float altMax = 10;
...
void setup() {
  ...
  if (useSerial) {
    tryInitSerial();
    // Initialize the baseline packet timer to current runtime clock at bootup
    lastPacketTime = millis();
  }
}

// Self-Healing Serial Connection Initialization Block
void tryInitSerial() {
  try {
    if (port != null) {
      println("Cleaning up active resources on " + serialPortName + "...");
      port.clear();
      port.stop(); 
      port = null;
    }
    ...
    println("Reconnection attempt failed on " + serialPortName + ": " + e.getMessage());
    port = null; // Ensure pointer remains null to retry safely next interval
  }
}
...
void checkSerialWatchdog() {
  ...
  if (timeSinceLastPacket > 2000) {
    ...
    // Attempt hardware reconnection every 2000ms if link drops
    if (millis() - lastReconnectAttempt > 2000) {
      println("🚨 CRITICAL LINK LOSS: Link timeout active (" + nf(timeSinceLastPacket/1000.0, 1, 1) + "s). Retrying hardware port bind...");
...
// ==========================================
// SIMPLE TEXT TELEMETRY PARSER (NEW FORMAT)
// ==========================================
void readSerialTelemetry() {
  // Safety Guard: Avoid reading from a null or uninitialized port handler
  if (port == null) return;
  ...
  try {
    ...
    while (port.available() > 0) {
      ...
      if (inString != null) {
        ...
        // BUG FIX: Only touch lastPacketTime if data is valid and populated!
        ...
        if (inString.startsWith("#")) continue;
        
        // Parse slash-delimited orientation data (roll/pitch/yaw)
        if (inString.contains("/")) {
          ...
          if (parts.length >= 3) {
            try {
              ...
              // Generate fake accel data from angles for display
              accelX = sin(radians(pitch)) * 0.5;
              accelY = sin(radians(roll)) * 0.5;
              accelZ = cos(radians(pitch)) * cos(radians(roll));
              ...
              if (packetCount == 1) {
                println("✓ First packet received! Roll=" + roll + " Pitch=" + pitch + " Yaw=" + yaw);
              }
            } catch (Exception e) {
        ...
        // Parse altitude (Alt(ft): 0.00)
        else if (inString.contains("Alt(ft):")) {
          try {
            String altStr = inString.replace("Alt(ft):", "").trim();
            ...
            println("Altitude parse error: " + inString);
          }
        }
      }
    }
  } catch (Exception e) {
    println("Serial dynamic read error: " + e.getMessage());
    port = null; // Flag connection loss immediately to kick-start the watchdog re-init
  }
}
...
// Render Rocket Nosecone - sits ON TOP of fuselage
pushMatrix();
translate(0, -length/2, 0);  // Move to top of fuselage
rotateX(PI);  // Rotate 180 degrees so tip points DOWN
fill(210, 50, 50); 
cone(radius, 65);  // Now cone points DOWN into the body
popMatrix();
...
void drawRightMetadataPanel() {
  ...
  // FIXED TIMEOUT CHECKING STRATEGY
  boolean isConnected = useSerial && (port != null) && (millis() - lastPacketTime < 2000);
...
void cone(float r, float h) {
  int sides = 32;
  // Draw cone pointing UP with BASE at y=0, TIP at y=h
  beginShape(TRIANGLE_FAN);
  vertex(0, h, 0);  // Tip at top (positive Y)
  for (int i = 0; i <= sides; i++) {
    float angle = TWO_PI * i / sides;
    vertex(cos(angle) * r, 0, sin(angle) * r);  // Base at y=0
  }
  endShape();
}
